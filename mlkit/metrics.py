...
# ---------------------------------------------------------------------------
# SINGLE SOURCE OF TRUTH for imbalance tiering.
# Cutoffs (on positive ratio), boundaries inclusive on the lower side:
#   balanced >= 0.40 | mild >= 0.20 | moderate >= 0.05 | severe >= 0.01 | extreme < 0.01
# Every caller (research, design, training) MUST use assign_tier — do not re-derive tiers.
# ---------------------------------------------------------------------------
TIER_CUTOFFS = ((0.40, "balanced"), (0.20, "mild"), (0.05, "moderate"), (0.01, "severe"))


def assign_tier(pos_ratio: float | None) -> str:
    """Map a positive ratio to an imbalance tier. The one place this logic lives."""
...
def detect_imbalance_tier(y) -> dict:
    ...
    return {
        ...
        "tier": assign_tier(pr), "scale_pos_weight": float(neg / max(pos, 1))
    }