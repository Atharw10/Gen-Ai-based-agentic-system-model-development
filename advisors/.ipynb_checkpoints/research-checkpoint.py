...
made and WHERE it came from (Tavily web URLs / Gemini / deterministic default).
...
Gemini fills the plan; if Tavily is configured, real web results are fetched first and cited.
"""
...
def _tavily_sources(query: str) -> list[str]:
    try:
...
def _llm_plan(objective: str, profile: ProfileResult, cached_llm, model_type: str | None = None) -> dict:
    ...
    tier = assign_tier(profile.target.pos_ratio)
    web_sources = _tavily_sources(f"banking ML best practices {objective} hyperparameters feature engineering") \
    ...
    prompt = (
        "You are a senior banking ML research assistant. Produce a RESEARCH PLAN as JSON for this "
        f"problem. Objective: {objective}. Positive rate: {profile.target.pos_ratio} "
        f"(imbalance tier: {tier}). Monthly snapshots: {profile.temporal.n_snapshots}.\n"
        + (f"Model type: {model_type}.\n" if model_type else "")
        + (f"Positive rate per snapshot: {pmr}. Drift check: {drift_note} "
           "When choosing observation_window_months and oot_months, prefer a recent window whose "
           "positive rate is stable; avoid spanning months whose positive rate differs sharply "
           "from the recent norm, and note in 'windows.reason' if the OOT period looks unusual.\n"
           if pmr else "")
        + (f"Cite these web sources where relevant: {web_sources}\n" if web_sources else "")
        + 'Return ONLY JSON with this schema (give concise reasons, valid algorithm names from '
    ...
    j = extract_json(cached_llm.invoke(prompt, label="research"))
    src = "Gemini" + (f" + web: {web_sources}" if web_sources else "")
    ...
    # 'deterministic default' source) is kept — we never stamp a hardcoded reason as if it were
    ...
    for key in ("models", "windows", "hpo", "feature_families"):
        ...
        if isinstance(sec, dict) and str(sec.get("reason", "")).strip():
            ...
            used_llm = True
    plan["sources"] = ([src] + web_sources) if (used_llm and web_sources) else ([src] if used_llm else plan["sources"])
