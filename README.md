GenAI-Based Agentic AI System for Automated Model Development
Overview
...
Key Features
Prompt-driven pipeline: Users provide only an objective and a dataset (CSV or other formats); the system handles profiling, cleaning, feature engineering, training, and evaluation automatically.
...
How It Works
User provides a modeling objective (e.g., "build a propensity model") and a dataset.
The LLM researcher node analyzes the objective and data to decide the algorithm family, OOT window, and training period.
Based on the algorithm selected, the pipeline routes to the appropriate training path (logistic regression or tree-based), each using a tailored prompt.
Human-in-the-loop checkpoints allow the user to accept or override key decisions.
The model is trained, evaluated, and results/metrics are logged via MLflow.
Use Case

Applied to build a propensity model for credit card upsell, predicting the likelihood of existing customers responding to a credit card upsell offer.
